...
func (s *Server) logout(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "已登出"})
}
...
func (s *Server) me(c *gin.Context) {
	...
	if s.knovisClient == nil || token == "" {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "Knovis 客户端未配置或缺少 token"})
		return
	}
	body, err := s.knovisClient.GetUser(c.Request.Context(), token, uid)
	if err != nil {
		log.Printf("[WARN][auth] /auth/me 透传 Knovis 失败 userID=%s: %v", uid, err)
		c.JSON(http.StatusBadGateway, gin.H{"error": "查询 Knovis 用户资料失败: " + err.Error()})
		return
	}
...
func (s *Server) setLLMKey(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "LLM key 已加密保存"})
}

// getLLMKey GET /auth/llm-key
// 返回是否已设置（不返回明文，安全考虑）
func (s *Server) getLLMKey(c *gin.Context) {
	uid, ok := ensureUserID(c)
	if !ok {
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"has_llm_key": has})
}

// deleteLLMKey DELETE /auth/llm-key
// 清除用户的 LLM key
func (s *Server) deleteLLMKey(c *gin.Context) {
	uid, ok := ensureUserID(c)
	if !ok {
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "LLM key 已删除"})
}
...
func (s *Server) setKnovisToken(c *gin.Context) {
	uid, ok := ensureUserID(c)
	if !ok {
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "Knovis token 已加密保存"})
}

// getKnovisToken GET /auth/knovis-token
// 返回是否已设置（不返回明文，安全考虑）
func (s *Server) getKnovisToken(c *gin.Context) {
	uid, ok := ensureUserID(c)
	if !ok {
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"has_knovis_token": has})
}

// deleteKnovisToken DELETE /auth/knovis-token
// 清除用户的 Knovis token
func (s *Server) deleteKnovisToken(c *gin.Context) {
	uid, ok := ensureUserID(c)
	if !ok {
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "Knovis token 已删除"})
}
