...
CREATE TABLE IF NOT EXISTS `agent_user_config` (
  ...
  `max_tool_rounds` INT NOT NULL DEFAULT 0 COMMENT '连续调用工具轮数上限（0=用全局默认10，范围1-50）',
  `sandbox_mode` VARCHAR(16) NOT NULL DEFAULT '' COMMENT '沙箱权限模式 ask/auto/yolo（空=ask）',
  `backup_mode` VARCHAR(16) NOT NULL DEFAULT '' COMMENT '文件备份模式 snapshot/git（空=snapshot）',
